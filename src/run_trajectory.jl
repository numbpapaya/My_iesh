...
function multiple_trajectory()
    ...
    Threads.@threads for traj in 1:numtraj
        ...
        create_dataset(fid, "v_no", s.storage_vno)
		...
		create_dataset(fid, "wavefunction", s.storage_psi)
		create_dataset(fid, "adiabatic states", s.storage_phi)
		create_dataset(fid, "x_au", s.storage_xau)
		create_dataset(fid, "v_au", s.storage_vau)
        close(fid)
    end
end
