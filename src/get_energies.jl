...
@inbounds @inline function get_E_au_n_coup(s::Simulation)::Float64
    ...
    for i in 1:N
        V = get_E_au_n_coup_loop!(s.x, V, i, delta_xn, cutoff_test, bool_cutoff_test)
    end
    return V
end
#---------------------------Au-O Coupling----------------------------------
...
@inline @inbounds function get_E_au_o_coup(s::Simulation)::Float64
    ...
    bool_cutoff_test = BitArray{1}(undef, 3)
    for i in 1:N
        V = get_E_au_o_coup_loop!(s.x, V, i, delta_xo, cutoff_test, bool_cutoff_test)
    end
    return V
end
#---------------------------Ionic----------------------------------
...
@inline @inbounds function get_E_au_o_ion(s::Simulation)::Float64
    ...
    bool_cutoff_test = BitArray{1}(undef, 3)
    for i in 1:N
        V = get_E_au_o_ion_loop(s.x, V, i, delta_xo, cutoff_test, bool_cutoff_test)
    end
    return V
end
...
@inline function get_E_au_n_ion(s::Simulation)::Float64
    ...
    bool_cutoff_test = BitArray{1}(undef, 3)
    for i in 1:N
        V = get_E_au_n_ion_loop(s.x, V, i, delta_xn, cutoff_test, bool_cutoff_test)
    end
    return V
end
...
@inbounds @inline function get_E_au_n_neutral(s::Simulation)::Float64
    ...
    bool_cutoff_test = BitArray{1}(undef, 3)
    for i in 1:N
        V = get_E_au_n_neutral_loop(s.x, i, V, delta_xn, cutoff_test, bool_cutoff_test)
    end
    return V
end

#---------------------------GOLD AU AU LATTICE----------------------------------
@inline function V_au_au_loop_if(x::float_array, V::Float64, i::Int, j::Int, xm, xi, temp,r, qf_1, qf_2)::Float64
    if s.nn_arr[i, j] != 0
        m = s.nn_arr[i, j] + 2
        @views xm .= x[m, :]
        @views xi .= x[i+2, :]
